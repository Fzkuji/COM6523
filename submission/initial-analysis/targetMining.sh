...
VERSIONS=($(git log --no-merges --pretty=format:"%h" $2))
...
for i in ${VERSIONS[@]}; do
    ...
    FILE_CHANGES=$(git show --numstat --no-renames --format="%n" $i)
    ...
    for k in ${arr[*]}; do
        echo $k
        echo "\"$DATE\", \"$MESSAGE\", \"$AUTHOR\", "${k} >> ${DATASTORE}
    done
